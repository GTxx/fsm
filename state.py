    ...
    def dispatch(self, context, event, obj):
        ...
        if context.currentState.exitAction:
            try:
                ...
                level('Error processing entry action for state. (Machine %s, State %s, exitAction %s)',
                      context.machineName, context.currentState.name, context.currentState.exitAction.__class__)
                raise
        ...
        if context.currentState.entryAction:
            try:
                ...
                if e.__class__ in TRANSIENT_ERRORS:
                    level = context.logger.warn
                level('Error processing entry action for state. (Machine %s, State %s, entryAction %s)',
                      context.machineName, context.currentState.name, context.currentState.entryAction.__class__)
                raise
        ...
        if context.currentState.isContinuation:
            try:
                ...
                if e.__class__ in TRANSIENT_ERRORS:
                    level = context.logger.warn
                level('Error processing continuation for state. (Machine %s, State %s, continuation %s)',
